...
use umber_model::capability::CapabilityKind;
use umber_model::catalog::{Catalog, CatalogSource};
...
use umber_ui::discovery::{DiscoverySession, UiModelEntry};
use umber_ui::schema::{SettingsDraft, SettingsPage};
use umber_ui::strings::Strings;

use umber_protocol::{OpenAiChatAdapter, ScriptedTransport};
...
fn host_journey_from_settings_to_streamed_answer() {
    ...
    let discovered = match session.state() {
        umber_ui::DiscoveryState::Done { models } => models.clone(),
    ...
    let catalog = Catalog {
        ...
        entries: vec![ModelProfile {
            identity: umber_model::identity::ModelIdentity {
            ...
            limits: umber_model::model::ModelLimits {
    ...
    assert_eq!(
        ...
        umber_model::capability::CapabilityStatus::Unknown
    );
    ...
    // 7. 断言最终产物
    umber_conformance::assert::check_monotonic(&events).unwrap();
    umber_conformance::assert::check_single_terminal(&events).unwrap();
