//! Provider Preset：真实规模的厂商预置（总案 §7 §31）。
//!
//! 设计要点（来自参考产品的现实）：
//! - 厂商有**几十个**，不是四个。而且官方 / 国内云厂商 / 聚合中转 / 本地部署
//!   是四类不同的东西，必须分组而不是平铺。
//! - **Provider ≠ Protocol**（§6）：绝大多数厂商说的是 openai_chat，
//!   因此"协议"对用户是次要显示项、对实现是首要区分项。
//! - Preset 只是默认值，端点永不被锁死（§34）。
//! - 本地部署无需 API Key（免密），必须能被清楚标注。
...
pub enum ProviderCategory {
    /// 官方直连（OpenAI / Anthropic / Google / DeepSeek …）。
    Official,
    /// 国内云厂商与模型厂商。
    China,
    /// 聚合中转（OpenRouter / SiliconFlow / 各类 relay）。
    Gateway,
...
impl ProviderOffering {
    const fn chat(endpoint: &'static str, models: &'static [&'static str]) -> Self {
        Self {
    ...
    const fn responses(endpoint: &'static str, models: &'static [&'static str]) -> Self {
        Self {
    ...
    const fn anthropic(endpoint: &'static str, models: &'static [&'static str]) -> Self {
        Self {
    ...
    const fn gemini(endpoint: &'static str, models: &'static [&'static str]) -> Self {
        Self {
...
pub struct ProviderPreset {
    ...
    /// 卡片头像用的单字母/短标识（我们没有各家 logo 授权）。
    pub badge: &'static str,
    pub offerings: &'static [ProviderOffering],
}

impl ProviderPreset {
    /// 首次出现（写在前面）的协议——即"默认协议"。
...
const OPENAI_M: &[&str] = &["gpt-4o-mini", "gpt-4o"];
const CLAUDE: &[&str] = &["claude-sonnet-4-5", "claude-3-5-haiku-latest"];
const GEMINI: &[&str] = &["gemini-2.5-flash", "gemini-2.5-pro"];
const GLM: &[&str] = &["glm-4.6", "glm-4.5-air"];
const KIMI: &[&str] = &["moonshot-v1-8k", "kimi-k2-turbo-preview"];
const QWEN: &[&str] = &["qwen-plus", "qwen-max", "qwen-turbo"];
const ERNIE: &[&str] = &["ernie-4.5-turbo-128k", "ernie-speed-128k"];
const HUNYUAN: &[&str] = &["hunyuan-turbos-latest", "hunyuan-lite"];
const MINIMAX: &[&str] = &["MiniMax-Text-01", "abab6.5s-chat"];
const STEP: &[&str] = &["step-2-16k", "step-1-flash"];
const SILICON: &[&str] = &["deepseek-ai/DeepSeek-V3", "Qwen/Qwen2.5-72B-Instruct"];
const MODELSCOPE: &[&str] = &["Qwen/Qwen3-235B-A22B", "deepseek-ai/DeepSeek-V3.1"];
const SENSENOVA: &[&str] = &["SenseChat-5", "SenseChat-Turbo"];
const ARK: &[&str] = &["doubao-seed-1-6-250615", "doubao-1-5-pro-32k"];
const XAI: &[&str] = &["grok-4", "grok-3-mini"];
const MISTRAL: &[&str] = &["mistral-large-latest", "mistral-small-latest"];
const GROQ: &[&str] = &["llama-3.3-70b-versatile", "qwen-2.5-32b"];
const TOGETHER: &[&str] = &["meta-llama/Llama-3.3-70B-Instruct-Turbo"];
const FIREWORKS: &[&str] = &["accounts/fireworks/models/llama-v3p3-70b-instruct"];
const NVIDIA: &[&str] = &["meta/llama-3.3-70b-instruct"];
const CEREBRAS: &[&str] = &["llama-3.3-70b"];
const PERPLEXITY: &[&str] = &["sonar-pro", "sonar"];
const OPENROUTER: &[&str] = &["deepseek/deepseek-chat", "anthropic/claude-sonnet-4.5"];
const OLLAMA: &[&str] = &["qwen3:8b", "llama3.2:3b"];
const LMS: &[&str] = &["local-model"];
const VLLM: &[&str] = &["your-served-model"];
...
pub const BUILTIN_PRESETS: &[ProviderPreset] = &[
    ...
    ProviderPreset {
        ...
        badge: "O",
        offerings: &[
            ProviderOffering::chat("https://api.openai.com/v1", OPENAI_M),
            ProviderOffering::responses("https://api.openai.com/v1", OPENAI_M),
        ],
    },
    ProviderPreset {
        id: "anthropic",
    ...
    ProviderPreset {
        ...
        badge: "D",
        offerings: &[
            ProviderOffering::chat("https://api.deepseek.com/v1", DEEPSEEK),
            ProviderOffering::anthropic("https://api.deepseek.com/anthropic", DEEPSEEK),
        ],
    },
    ProviderPreset {
        id: "xai",
        ...
        badge: "X",
        offerings: &[ProviderOffering::chat("https://api.x.ai/v1", XAI)],
    },
    ProviderPreset {
        id: "mistral",
        ...
        badge: "M",
        offerings: &[ProviderOffering::chat("https://api.mistral.ai/v1", MISTRAL)],
    },
    ProviderPreset {
        id: "groq",
    ...
    ProviderPreset {
        ...
        badge: "Z",
        offerings: &[ProviderOffering::chat("https://api.z.ai/api/paas/v4", GLM)],
    },
    ProviderPreset {
        id: "moonshot",
        ...
        badge: "K",
        offerings: &[ProviderOffering::chat("https://api.moonshot.cn/v1", KIMI)],
    },
    ProviderPreset {
        id: "dashscope",
    ...
    ProviderPreset {
        ...
        badge: "阶",
        offerings: &[ProviderOffering::chat("https://api.stepfun.com/v1", STEP)],
    },
    ProviderPreset {
        id: "sensenova",
    ...
    ProviderPreset {
        ...
        badge: "OL",
        offerings: &[ProviderOffering::chat("http://localhost:11434/v1", OLLAMA)],
    },
    ProviderPreset {
        id: "lmstudio",
        ...
        badge: "LM",
        offerings: &[ProviderOffering::chat("http://localhost:1234/v1", LMS)],
    },
    ProviderPreset {
        id: "vllm",
        ...
        badge: "vL",
        offerings: &[ProviderOffering::chat("http://localhost:8000/v1", VLLM)],
    },
    // ---------- 自定义 ----------
    ProviderPreset {
        id: "custom",
...
/// 按分类分组（UI 分组渲染用），保持 BUILTIN_PRESETS 内的相对顺序。
...
/// 搜索过滤（匹配 id / 名称文案 key 的尾部 / badge）。
///
/// 名称是 i18n key，因此这里匹配 key 的语义部分与 id——
/// UI 层再按当前语言的名称做一次匹配（见 `search_presets_localized`）。
...
mod tests {
    ...
    #[test]
    fn preset_count_matches_real_world_scale() {
        // 现实是几十个厂商，不是四个
        assert!(
            BUILTIN_PRESETS.len() >= 25,
            "厂商预置数量不足：{}",
            BUILTIN_PRESETS.len()
        );
        let groups = presets_by_category();
        assert!(groups.iter().any(|(c, _)| *c == ProviderCategory::Official));
        assert!(groups.iter().any(|(c, _)| *c == ProviderCategory::China));
        assert!(groups.iter().any(|(c, _)| *c == ProviderCategory::Gateway));
        assert!(groups.iter().any(|(c, _)| *c == ProviderCategory::Local));
        assert!(groups.iter().any(|(c, _)| *c == ProviderCategory::Custom));
    }

    #[test]
    fn every_preset_has_at_least_one_usable_offering() {
        for preset in BUILTIN_PRESETS {
            assert!(!preset.offerings.is_empty(), "{} 没有 offering", preset.id);
            // 自定义厂商的地址留空由用户填写，其余必须有默认地址
    ...
    #[test]
    fn local_presets_are_keyless_and_have_no_key_url() {
        for preset in BUILTIN_PRESETS
            .iter()
            .filter(|p| p.category == ProviderCategory::Local)
        {
            assert!(preset.keyless, "{} 本地部署应免密", preset.id);
            assert!(
                preset.key_url.is_none(),
                "{} 本地部署不该有申 Key 链接",
                preset.id
            );
        }
    ...
    #[test]
    fn non_local_presets_provide_key_url_and_recommendations() {
        for preset in BUILTIN_PRESETS.iter().filter(|p| {
            ...
            assert!(
                preset
                    .offerings
                    .iter()
                    .all(|o| !o.recommended_models.is_empty()),
                "{} 缺少推荐模型",
                preset.id
            );
        }
