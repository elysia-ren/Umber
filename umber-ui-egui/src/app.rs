...
//!
//! 渲染纪律：
//! - 逻辑全在 `umber_ui::SettingsState`，本文件只画界面、转交事件
...
enum JobResult {
    Connection(Result<umber_ui::ConnectionReport, BackendError>),
    Discovery(Result<Vec<UiModelEntry>, BackendError>),
    Save(Result<umber_ui::SaveReport, BackendError>),
}
...
impl SettingsApp {
    ...
    fn model_knowledge(&mut self, ui: &mut Ui) {
        ...
        // 能力
        let capabilities: Vec<(umber_ui::CapabilityKind, CapabilityStatus)> = self
            .state
            .selected_profile()
...
fn capability_short(kind: umber_ui::CapabilityKind) -> &'static str {
    use umber_ui::CapabilityKind::*;
    match kind {
        Text => "文",
        Vision => "视",
        Audio => "听",
...
mod tests {
    ...
    fn every_category_has_a_title_key() {
        for preset in umber_ui::BUILTIN_PRESETS {
