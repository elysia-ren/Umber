...
//! - 只依赖 `umber-ui` 的数据类型（SettingsPage / DiscoverySession /
...
pub struct SettingsWindowParams {
    ...
    pub scale: f32,
    pub density: Density,
    pub page: umber_ui::SettingsPage,
...
mod backend {
    use umber_ui::{BackendError, ConnectionReport, SettingsBackend, SettingsDraft};
    ...
    impl SettingsBackend for NullBackend {
        ...
        fn discover(
            ...
            _: &SettingsDraft,
            _: Option<&str>,
        ) -> Result<Vec<umber_ui::UiModelEntry>, BackendError> {
...
pub fn open_settings_window(params: SettingsWindowParams) -> Result<(), eframe::Error> {
    ...
    eframe::run_native(
        ...
        options,
        Box::new(move |cc| {
            let strings = umber_ui::Strings::builtin(params.language)
                .unwrap_or_else(|| umber_ui::Strings::builtin("zh-CN").expect("zh-CN exists"));
