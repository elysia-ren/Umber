...
use runtime_core::event::SequencedEvent;
...
) -> (Vec<SequencedEvent>, runtime_engine::InvocationOutcome) {
...
fn report(events: &[SequencedEvent], outcome: &runtime_engine::InvocationOutcome, label: &str) {
...
fn live_deepseek_streaming() {
    ...
    tool_request.tool_choice = runtime_core::request::ToolChoice::Required;
    let factory = {
...
fn live_deepseek_cancellation_mid_stream() {
    ...
    assert!(
        matches!(terminal.event, runtime_core::event::ModelEvent::Cancelled),
...
fn real_transport_round_trips_https() {
    ...
    match response {
        ...
        runtime_protocol::HttpResponse::Text { status, body } => {
