...
use runtime_core::request::{GenerateRequest, ReasoningConfig, ReasoningEffort, ResponseFormat};
...
fn deployment() -> Deployment {
    Deployment {
        ...
        protocol: runtime_model::deployment::ProtocolKind::Gemini,
...
fn fixture_prompt_blocked_maps_to_content_filtered() {
    ...
    match &terminal.event {
        runtime_core::event::ModelEvent::Failed { error } => {
...
fn tool_result_uses_function_response_with_name() {
    let history = Message::assistant(vec![runtime_core::content::ContentBlock::ToolCall(
        runtime_core::content::ToolCallBlock {
            call_id: "call-1".into(),
    ...
    let tool_result = Message::new(
        ...
        vec![runtime_core::content::ContentBlock::ToolResult(
            runtime_core::content::ToolResultBlock {
                call_id: "call-1".into(),
                is_error: false,
                content: vec![runtime_core::content::ContentBlock::text("晴 28°C")],
            },
        )],
    );
