...
use runtime_model::probe::ProbeResult;
...
impl LocalDb {
    ...
    pub fn load_registry(&self) -> Result<ModelRegistry, StoreError> {
        ...
        for deployment in self.deployments()? {
            let deployment_model = runtime_model::deployment::Deployment {
                id: deployment.id.clone(),
                endpoint_id: deployment.endpoint.clone(),
                protocol: match deployment.protocol.as_str() {
                    "openai_responses" => runtime_model::deployment::ProtocolKind::OpenAiResponses,
                    "anthropic_messages" => {
                        runtime_model::deployment::ProtocolKind::AnthropicMessages
                    }
                    "gemini" => runtime_model::deployment::ProtocolKind::Gemini,
                    _ => runtime_model::deployment::ProtocolKind::OpenAiChat,
                },
...
mod tests {
    ...
    use runtime_model::model::{ModelLimits, ModelProfile, ParameterSupport};
    use runtime_model::probe::{ProbeResult, ProbeTestType};
