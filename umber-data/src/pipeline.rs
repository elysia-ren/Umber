...
use umber_model::catalog::{Catalog, CatalogDeployment, CatalogSource};
use umber_model::compatibility::CompatibilityProfile;
use umber_model::evidence::{Evidence, EvidenceSource};
use umber_model::identity::{normalize_model_id, ModelIdentity};
use umber_model::model::{
    ModelLimits, ModelModalities, ModelProfile, ParameterSupport, Pricing, ReasoningInfo,
    ToolSupport,
};
use umber_model::resolver::{self, FieldCandidate, FieldCategory, FieldValue};
...
pub struct FieldCandidates {
    ...
    pub modalities_in: Vec<(umber_model::model::Modality, EvidenceSource)>,
    pub modalities_out: Vec<(umber_model::model::Modality, EvidenceSource)>,
    pub capabilities: Vec<(CapabilityKind, bool, EvidenceSource)>,
    pub supported_efforts: Vec<(umber_core::request::ReasoningEffort, EvidenceSource)>,
    pub default_effort: Vec<(umber_core::request::ReasoningEffort, EvidenceSource)>,
...
pub fn normalize(record: &RawModelRecord) -> FieldCandidates {
    ...
    let supported_efforts =
        umber_model::effort::efforts_from_labels(&record.reasoning_effort_labels)
            .into_iter()
    ...
    let default_effort = record
        ...
        .as_deref()
        .and_then(umber_model::effort::from_source_label)
        .map(|e| (e, source.clone()))
        .into_iter()
        .collect();
...
pub fn resolve_group(group: &[FieldCandidates]) -> (ModelProfile, bool) {
    ...
    let mut in_modalities: Vec<umber_model::model::Modality> = group
        .iter()
        .flat_map(|c| c.modalities_in.iter().map(|(m, _)| *m))
        .collect();
    in_modalities.sort_unstable();
    in_modalities.dedup();
    let mut out_modalities: Vec<umber_model::model::Modality> = group
        .iter()
    ...
    let mut efforts: Vec<umber_core::request::ReasoningEffort> = group
        .iter()
    ...
    let distinct_effort_sets = {
        let mut sets: Vec<Vec<umber_core::request::ReasoningEffort>> = group
            .iter()
            .map(|c| {
...
fn finalize_derived_fields(mut profile: ModelProfile) -> ModelProfile {
    ...
    profile.structured_output = umber_model::model::StructuredOutputInfo {
        json_mode,
        json_schema,
    };
...
mod tests {
    ...
    fn effort_labels_from_openrouter_shape_merge_into_profile() {
        ...
        assert_eq!(
            ...
            vec![
                umber_core::request::ReasoningEffort::Low,
                umber_core::request::ReasoningEffort::High
            ]
        );
        assert_eq!(
            profile.reasoning.default_effort,
            Some(umber_core::request::ReasoningEffort::High)
        );
        // 就近降级：请求 Medium → 实际 Low
        let resolved = umber_model::effort::nearest(
            umber_core::request::ReasoningEffort::Medium,
            &profile.reasoning.supported_efforts,
        )
        .unwrap();
        assert_eq!(
