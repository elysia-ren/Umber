...
use umber_data::licenses::{self, SourceLicense};
use umber_data::sources::{
    LitellmAdapter, ModelsDevAdapter, OfficialOverlay, OpenRouterAdapter, SourceAdapter,
};
use umber_data::{build, LocalDb, RawModelRecord};
...
fn main() -> ExitCode {
    ...
    match command {
        ...
        "inspect" => {
            ...
            let out: umber_data::BuildOutput = match serde_json::from_str(&text) {
                Ok(v) => v,
                Err(e) => {
            ...
            let meta: BTreeMap<&str, &umber_data::RecordMeta> = out
                .records
                .iter()
