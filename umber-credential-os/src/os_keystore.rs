...
use umber_credential::{CredentialError, CredentialRef, CredentialStore, SecretString};

/// 钥匙串中的服务名（Windows 的 target、macOS 的 service）。
pub const SERVICE: &str = "Umber";
...
mod tests {
    ...
    /// `cargo test -p umber-credential-os -- --ignored`
    #[test]
    #[ignore = "writes to the real OS keystore"]
    fn os_keystore_round_trip() {
        let store = OsKeystore::with_service("UmberTest");
