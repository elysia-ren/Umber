...
# ---- ABI 常量（与 umber-ffi/src/lib.rs 一致）----
ABI_MAJOR = 0
ABI_MINOR = 2
...
def _library_candidates() -> list[str]:
    ...
    - 源码仓库：``<repo>/umber-ffi/bindings/python/umber.py``
      → 库在 ``<repo>/target/{debug,release}``
    - 集成包：``<pkg>/bindings/python/umber.py``
      → 库在 ``<pkg>/lib``

    生产环境建议显式传路径（``Runtime(path)``），别依赖自动查找。
    """
    names = ["umber_ffi.dll", "libumber_ffi.so", "libumber_ffi.dylib"]
    here = os.path.dirname(os.path.abspath(__file__))
    roots = [
        # 集成包布局：bindings/python -> <pkg>/lib
        os.path.join(here, "..", "..", "lib"),
        os.path.join(here, "..", "lib"),
        # 源码仓库布局：umber-ffi/bindings/python -> <repo>/target/*
...
def load_library(path: Optional[str] = None):
    ...
    raise FileNotFoundError(
        "umber_ffi shared library not found. Looked in: "
        + ", ".join(_library_candidates())
        + ". Pass an explicit path, e.g. Runtime(r'<pkg>/lib/umber_ffi.dll')."
    )
