//! M1 引擎集成测试：用 runtime-conformance 的 fake provider 验证
//! 总案 §23–§31 的全部硬保证。

use std::time::Duration;

use runtime_conformance::assert as cassert;
use runtime_conformance::fake::{Script, ScriptedFactory};
use runtime_core::content::TextBlock;
use runtime_core::error::{ErrorDetail, ModelError, TimeoutKind};
use runtime_core::event::{ModelEvent, SequencedEvent};
use runtime_core::ids::{BlockId, InvocationId};
...
fn completed(invocation_id: &str, text: &str) -> ModelEvent {
    ModelEvent::Completed {
        response: Box::new(GenerateResponse {
            ...
            content: vec![runtime_core::ContentBlock::Text(TextBlock::new(text))],
